...
_U={"s":1000.0,"ms":1.0,"us":0.001,"µs":0.001,"μs":0.001,"ns":1e-6}
def to_ms(v):
    s=str(v).strip()
...
def mean_series(key):
    ...
    df=read_csv_robust(find_csv(key)); df["Time"]=pd.to_datetime(df["Time"])
...
path=os.path.join(OUT_DIR,"delay_overall_avg_aligned.csv")
