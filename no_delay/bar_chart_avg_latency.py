...
CSV_PATH = Path(r"average_latency\overall_avg_aligned.csv")

OUT_IMAGE = Path(r"average_latency\avg_latency_bar_chart.png")
OUT_SUMMARY = Path(r"average_latency\avg_latency_summary_bar.csv")

TITLE = "Întârziere medie"
Y_LABEL = "Întârziere medie (ms)"

# Dacă ai rânduri artificiale cu 0 la început, le scoate din medie
REMOVE_ZERO_ROWS = True

# Dacă vrei barele sortate crescător după latență
SORT_BARS = False
...
# Convertim toate coloanele de latență în numeric
for col in latency_columns:
    df[col] = pd.to_numeric(df[col], errors="coerce")

# Scoatem rândurile unde toate latențele sunt 0
...
summary.columns = ["planificator", "intarziere_medie_ms"]
...
ax.set_xlabel("Planificator", fontsize=12, fontweight="bold")
